...
> 문자열 "Hello" 를 생성할 때 내부적으로는 문자열들의 구분을 위해 뒤에 null(\\0) 값이 붙는다.
...
> * 메서드 오버라이딩: 자식 객체가 부모타입으로 참조 될 때, 실행 시점에 오버라이딩 된 메서드를 선택
> * 동적 바인딩: 자바에서 모든 메서드 호출은 실행 시점에 결정
> Dynamic dispatch 는 객체 지향 언어의 주요 개념인 다형성을 구현하기 위한 핵심이지만, 이 과정에서 비용이 발생
