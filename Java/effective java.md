...
> * 박싱된 기본 타입은 null 이 허용가능하다.
```java
// 박싱된 타입의 초기 값은 null 이 기 때문에 NullPointerException 발생
Integer i;
if (i == 10) // 특정 동작 
```
> * 박싱된 기본 타입은 == 동등성 비교에서 객체 식별성 비교가 이루어질 수 있다.
```java
// 두 값은 같으나 인스턴스가 다름으로 false
// 이로 인해 비교 메서드 등 에서도 문제가 생길 수 있다.
if (new Integer(10) == new Integer(10)) 
```
> * 박싱된 기본 타입은 기본타입보다 시간, 메모리를 더 많이 사용한다.
> * 오토박싱이 되지만, 이는 리소스를 사용하는 작업이다.
```java
Long l = 0L;
for(long i = 0; i <= Integer.MAX_VALUE; i++){
	// 계속 오토박싱이 발생해서 속도가 저하된다.
	l += i;
}
```
> 제네릭을 사용할 때나, 리플렉션을 사용할 때는 기본타입을 사용할 수 없으므로, 박싱된 기본타입을 사용한다.
#### i.62	다른 타입이 적절하다면 문자열 사용을 피하라
> 문자열을 만능 자료형처럼 사용하지 마라.
> * 열거 타입 대신에 사용하지 마라([i.34](https://github.com/ehbong/Study/blob/main/Java/effective%20java.md#i34int-%EC%83%81%EC%88%98-%EB%8C%80%EC%8B%A0-%EC%97%B4%EA%B1%B0-%ED%83%80%EC%9E%85%EC%9D%84-%EC%82%AC%EC%9A%A9%ED%95%98%EB%9D%BC))
> * 혼합 타입 대신에 사용하지마라
```java
// 1개이상의 의미, 타입 등을 가진 데이터를 합쳐서 사용하면 안된다.
// # 이 합쳐지는 값 중에 포함되면 사용할 수 없는 값이 된다.
// 비교, 정렬 등 기능을 사용하기 어렵다.
...
> 자바 직렬화는 객체 또는 데이터를 바이트 형태로 변환하고, 다시 복구하는 방법이다.
> * 역직렬화는 복구되는 데이터가 어떤 형태인지 사전에 알 수 없기 때문에, 위험하다.
> * 위험을 피하는 가장 좋은 방법은 사용하지 않는 것이고(대안: JSON, 프로토콜 버퍼),  
> 꼭 사용해야 한다면 필터링을 사용해야한다.
> * [직렬화 설명링크](https://techblog.woowahan.com/2550/)
