...
|동시성|병렬성|
|----|----|
|교차실행을 통해 동시에 실행되는 것처럼 구현한 것(멀티태스킹)|실제로 동시에 처리되는 것|
|싱글코어에서 스레드를 활용해서 구현|멀티 코어에서 스레드로 구현|
|한번에 1개의 일을 처리하지만 교차 실행|동시에 많은 일을 처리|
|논리적인 개념|물리적인 개념|
|IO작업에 유리|CPU연산에 유리|

> * 병렬성은 동시성을 포함한 개념  
>   멀티 코어에서도 하나의 코어에서 여러 작업을 교차해서 하는 동시성 작업이 발생할 수 있음


> 동기, 비동기, 블로킹IO, 논블로킹IO
> * 동기, 비동기: 완료를 기다리느냐 특정 이벤트(콜백)를 받아 진행하느냐의 차이
> * 블로킹, 논블로킹: 제어권을 넘겨서 작업이 멈추느냐, 제어권을 유지 하느냐의 차이
