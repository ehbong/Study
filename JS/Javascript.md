...
4. 이벤트 큐(Event Queue):
이벤트 큐는 비동기 함수의 콜백 함수 또는 이벤트 리스너의 발생한 이벤트들이 대기하는 큐 자료 구조의 공간

5. 이벤트 루프(Event Loop):
이벤트 루프는 호출 스택과 이벤트 큐를 모니터링하고, 호출 스택이 비어있을 때 이벤트 큐에 있는 함수들을 호출 스택으로 옮기는 역할
이벤트 루프는 호출 스택이 비어있는지 반복적으로 확인하며, 비어있으면 이벤트 큐에서 함수를 하나씩 꺼내어 호출 스택으로 옮김
```
