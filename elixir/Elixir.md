...
>프로세스
>* Erlang VM 위에서 사용되는 프로세스 들로, 운영체제의 프로세스가 아닌 VM 위에 생성되는 경량(가상) 프로세스
>* 상호 독립적이며, 객체지향언어의 객체에 가깝게 사용


#### [노드](https://hexdocs.pm/elixir/1.12/Node.html)
>얼랭 VM, 운영체제 위에서 동작하는 작은 운영체제
>* 이벤트,스케줄링, 메모리, 등을 다룬다.
>* 노드 간 서로 연결이 가능(로컬 및 통신이 가능한 노드들 끼리 연결)
>* 쿠키 값으로 쿠키가 같은 값끼리만 연결(보안)

>노드 기본 명령어
```elixir
Node.self/0 # 자신 노드 이름을 출력 `이름@컴퓨터이름` 의 구조, 노드 이름은 아톰
Node.list/0 # 연결된 노드들 목록(자신 제외)
Node.connect/1 :"노드 이름" # 노드 연결, 아톰이므로 : + @를 포함해서 큰따옴표로 감싸서 입력
Node.disconnect/1 :"노드 이름" # 노드 연결 해제
Node.spawn/3 :"노드이름", 함수, 인자 # 다른 노드에 특정 함수를 실행하는 프로세스 생성
